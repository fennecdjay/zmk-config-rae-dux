...
#define MACRO(name, wms, tms, k)                                                    \
/ { macros { ZMK_MACRO(name, wait-ms = <wms>; tap-ms = <tms>; bindings = <k>;) };};

#define TAP_DANCE(name, ttms, k0, k1) \
ZMK_BEHAVIOR(name, tap_dance,         \
    tapping-term-ms = <200>;          \
    bindings = <k0>, <k1>;            \
)

#define HOLD_TAP(name, type, ttms, qtms, k0, k1) \
